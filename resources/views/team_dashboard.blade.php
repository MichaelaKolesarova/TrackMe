...
@section('content')
            ...
            <div class="dropdown-menu" aria-labelledby="userDropdown">
                @foreach(User::all() as $user)
                    @if($user->id != auth()->id() && $user->id != $chosenUser)
                        <a class="dropdown-item" href="#"  onclick="updateChosenUser({{$user->id}})" user_id="{{$user->id}}">{{ $user->name }}</a>
            ...
            <div class="col-sm-12 col-md-6 col-lg-3 mb-4">
                <div class="card tasks-card">
                    <div class="card-body">
                        <div>
                            <span class="card-title-text">TO DO</span>
                            <span class="badge bg-secondary right brown">{{Task::all()->where('assignee', $chosenUser)->where('taskStatus', TaskStatusEnum::ToDo->value)->count() }} tasks</span>
                        </div>

                        <div taskStatus="{{TaskStatusEnum::ToDo}}" id="card-tasks-todo" class="min-height">

                            @foreach(Task::all()->where('assignee', $chosenUser)->where('taskStatus', TaskStatusEnum::ToDo->value) as $task)
                                <div id="{{$task->id}}" class="task-card" draggable="true">
                                    <div class="task-name fill-width row">
                                        <a href="{{ route('task.overview', ['task' => $task->id]) }}"
                                           class="task-title task-link" draggable="false">
    ...
    <script>
        function updateChosenUser(userId) {
            ...
            document.getElementById('chosenUserName').innerText = document.getElementById('user-' + userId).innerText;
